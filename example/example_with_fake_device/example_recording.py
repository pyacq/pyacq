...
def test1():
    ...
    # Read the files
    reader = neo.RawBinarySignalIO(filename = os.path.join(dirname, 'Analog fake 0.raw'))
    ...
    analogsignals2 = [ ]
    arr = np.memmap(filename = os.path.join(dirname, u'Digital fake 1 éà.raw'), mode = 'r', dtype = np.uint8)
    arr = arr.reshape(-1, 2)
    print arr.shape
